...
两阶段执行把输入和 CPU Golden 的生成，与目标设备执行拆开，适用于准备机和执行机
不是同一台服务器的场景。

```text
prepare：CSV -> input 生成/修正 -> CPU Golden -> 保存
replay： CSV -> 恢复 input/Golden -> NPU API 或 Kernel -> compare
```
...
| prepare | `--no-prof --dump in,golden` | 生成并保存 | 生成并保存 | 跳过 | 跳过 |
| replay | `--manual-data-dirs DIR...` | 从文件恢复 | 从文件恢复 | 执行 | 执行 |

prepare 目前只支持 input 和 Golden 一起生成，不提供单独的 input 或 Golden 子阶段。
...
同名 case 重新 prepare 时，旧目录先失效。新数据在隐藏临时目录中写入并回读校验，
...
- input plugin；
- CPU Golden API 或 Golden plugin。
...
- prepare 禁止 output/full dump、`print`、`--dump-on-fail`、`--golden-mode Disable`、
  `--validate`，E2E 还禁止 graph 选项，Kernel 还禁止 `--compile-only`；
