...
两阶段执行把输入、可选 CPU Golden 和目标设备执行解耦，适用于准备机和执行机
不是同一台服务器，以及需要先单独生成 input 的场景。

```text
prepare input：  CSV -> input 生成/修正 -> 只保存 input
prepare 完整数据：CSV -> input 生成/修正 -> CPU Golden -> 同时保存
...
| replay | `--manual-data-dirs DIR...` | 从文件恢复 | 有文件则恢复；没有则在设备执行后生成 | 执行 | 执行 |

E2E 和 ACLNN 支持 input-only 和完整 prepare。input-only 会替换同名 case 的整个目录，避免新 input 误配旧 Golden
...
`--no-prof --dump in,golden` 才进入本流程的 prepare。Kernel 不支持单独 `--dump in`：
它可能需要 Golden 或动态输出信息在设备执行前分配 output buffer，无法
保持 E2E/ACLNN 的“先 NPU、后补 Golden”语义。
...
同名 case 重新准备 input 时，旧目录先失效，因此旧 Golden 不会残留。新文件在隐藏临时
目录中写入并回读，所选数据全部校验成功后才发布，不会留下可 replay 的半成品。

## 4. 文件协议

一个完整 case 可以包含以下文件；input-only 目录不包含任何`golden_*`文件：
...
replay 始终要求 input/scalar 槽位完整并跳过：

- 随机输入生成和 attributes 数据覆盖；
- input plugin。

Golden 文件是可选的。存在时，TTK 在设备输出 shape 可用后加载并校验；完全不存在时，
TTK 先执行 NPU，再用恢复的 input 运行 CPU Golden，最后 compare。Golden 索引不连续、
槽位数不匹配或 shape/dtype 错误仍会明确失败，不会把损坏的部分数据当成“缺失”。
...
ACLNN/E2E TestSpec可以声明`pre_npu`，处理最终input生成/恢复之后、主设备API之前必须完成的
算子自定义动作。prepare不执行该阶段；普通执行和replay在设备锁内各执行一次。

hook保留普通API形式的`kwargs`，其中包括非API CSV属性；只有显式声明`context: TtkContext`时才会额外获得
恢复后的tensor/scalar、只读attributes和CSV字段、manual-data根目录、当前case目录以及进程内state。
返回`PreNpuResult(stop=True)`后，本case会在主API、Golden和compare之前结束。ACLNN模式可在hook执行期间
通过`context.run_aclnn(...)`使用框架托管的辅助ACLNN device/stream/executor；ACLNN/E2E都可通过
`context.run_profiled(...)`按本轮`--task-prof`、`--warmup`和`--run`采集算子自定义动作。profile使用固定
`msprof/pre_npu/<case>/<stage>/`路径；后续同case/stage运行可以覆盖旧内容，使旧`RuntimeProfile.result_path`
不再指向当时的数据。两个runner都只在pre-NPU hook生命周期内有效。

`context.manual_case_dir`在当前 testcase 目录可唯一确定时暴露该目录。它只是通用路径，不是算子状态 API；
TTK 不规定插件文件名、扩展名、schema 或生命周期。
ACLNN/E2E的input、Golden和compare可以各自显式声明带`TtkContext`类型标注的`context`接收同一个运行时对象；
未标注的历史业务`context`和只声明`**kwargs`的老hook保持原有调用，Kernel/GEIR当前不注入该context。
完整合同见`ttk/test_spec/README.md`。
...
- prepare 禁止 output/full dump、`print`、`--dump-on-fail`和`--validate`；包含 Golden 的
  prepare 禁止`--golden-mode Disable`，input-only 允许；E2E 还禁止 graph 选项，Kernel
  只接受`--dump in,golden`且禁止`--compile-only`；
...
| 两种 prepare | E2E/ACLNN 的 input-only、完整 prepare 均不执行设备 |
| 缺失 Golden | input-only 数据直接 replay，确认设备执行后生成 Golden 并 compare |
...
严格框架数据协议不使用manifest；TTK校验自身数据文件，但不检查插件普通附属文件的payload，因此不能自动校验：
...
分阶段模式通常要求Golden是恢复input的纯函数。若assets的Golden依赖
`customize_inputs` 在进程内写入的全局缓存（而不是依赖传入 tensor/attributes），另一个
进程中的“缺失 Golden replay”无法重建该私有状态。框架不会为了兼容这种
副作用而再次执行 input plugin，因为那会违背“文件 input 是唯一设备输入”的 replay 合同。
这类assets应继续使用完整`--dump in,golden`、把Golden改为只依赖显式输入，或在`manual_case_dir`下由算子自行
保存带版本的必要状态。
...
| `manual-data framework entry is not a regular file` | 完整匹配框架数据命名的条目必须是普通文件，不能是目录或软链 |
