...
def match_overload(api_name, input_tensor_count, attributes=None, tensor_distribution=None, api_info=None):
    ...
    for oidx, ov_info in enumerate(info.overloads):
        ...
        input_tensors = [p for p in overload_params
        ...
        if tensor_distribution is not None:
            type_ok = True
            non_var_tensors = [p for p in input_tensors
    ...
    if len(tied) > 1:
        value_scored = [(ks, score_attr_type_compatibility(op, attr_values), oidx, op)
                        for ks, oidx, op in tied]
...
def coerce_value(raw, target_type):
    ...
    if '|' in target_type:
        ...
        for member_type in target_type.split('|'):
            try:
                ...
                errors.append(str(e))
        raise ValueError(
            f"Cannot coerce {raw!r} to union type {target_type}: "
            f"none of the members succeeded: {'; '.join(errors)}")
    if raw is None:
        return None
    ...
    if target_type in ('float', 'Number', 'Scalar'):
        ...
        return float(raw)
    if target_type == 'str':
        if isinstance(raw, str) and len(raw) >= 2 and ((raw[0] == '"' and raw[-1] == '"') or (raw[0] == "'" and raw[-1] == "'")):
            return raw[1:-1]
        return str(raw)
...
class ParamPlan:
    ...
    __slots__ = (
        'api_name', 'overload_params', 'overload_index',
        'output_tensor_indexes', 'attributes',
    )

    def __init__(self, api_name, overload_params, overload_index,
                 output_tensor_indexes, attributes):
    ...
    def build_args(self, nested_tensors):
        ...
        is_inplace = (len(parts) >= 3 and parts[0] == 'torch' and parts[1] == 'Tensor'
...
def build_positional_args(api_name, nested_tensors, attributes,
                          output_tensor_indexes, tensor_distribution=None, api_info=None):
    ...
    overload_params, oidx = match_overload(
        ...
        input_tensor_count=sum(
            1 for i, _ in enumerate(nested_tensors)
            if i not in set(output_tensor_indexes or ())),
    ...
    plan = ParamPlan(api_name, overload_params, oidx,
                     output_tensor_indexes, attributes)
    args, kwargs, _ = plan.build_args(nested_tensors)
