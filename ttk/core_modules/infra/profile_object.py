...
class ProfileObject(metaclass=ABCMeta):
    ...
    def pre_exit(self):  # noqa: B027
        pass

    def apply_compile_fail_result(self, testcase: TestcaseBase, crash_info: str, task_sub_type: str):  # noqa: B027
        pass

    def apply_compile_success_result(self, testcase: TestcaseBase, result: Any):  # noqa: B027
        pass

    def compile_done(self, testcase: TestcaseBase) -> Optional[tuple]:
        return None

    def handle_task_result_system_error(
        self, task: TaskA, result: SystemError, last_stage: str, pid: int
    ) -> Optional[tuple]:
    ...
    def output_titles(self, testcase: TestcaseBase, case_original_headers: list) -> tuple:
        ...
        if self.switches.custom_columns:
            result_titles = tuple(title for title in result_titles if title in self.switches.custom_columns)
            input_titles = tuple(title for title in input_titles if title in self.switches.custom_columns)
    ...
    def _compile_crash(self, task: TaskA, result: SystemError, last_stage: str, pid: int):
        if not issubclass(task.testcase.__class__, TestcaseBase):
            raise RuntimeError(f"Only subclass of TestcaseBase instance is valid. But got {task.testcase.__class__}")
        logging.fatal(
            f"Compilation process of mode {task.sub_type} crashed at stage {last_stage} "
    ...
    def _compile_fail(self, task: TaskA, result: RuntimeError, pid: int):
        ...
        logging.error(
            f"Compilation process of mode {task.sub_type} failed "
            f"for testcase {task.testcase.testcase_name} with pid {pid}, "
            f"fail reason: \n{exception_print}"
        )
    ...
    def _profile_crash(self, task: TaskA, result: SystemError, last_stage: str, pid: int) -> tuple:
        if not issubclass(task.testcase.__class__, TestcaseBase):
            raise RuntimeError(f"Only subclass of TestcaseBase instance is valid. But got {task.testcase.__class__}")
        logging.fatal(
            f"Profile process crashed at stage {last_stage} "
            f"for testcase {task.testcase.testcase_name} with pid {pid}. "
            f"System error: {result}"
        )
        results = self._profile_fail_result("PROFILE_CRASH", f"Crashed at profiling stage: {result}")
    ...
    def _profile_fail(self, task: TaskA, result: RuntimeError, pid: int) -> tuple:
        ...
        logging.error(
            f"Profiling process failed "
            f"for testcase {task.testcase.testcase_name} with pid {pid}, "
            f"fail reason:\n{exception}"
        )
        results = self._profile_fail_result("FAILURE", exception)
        inputs = task.testcase.pick_data(self.case_input_title)
        return self._assemble_row(inputs, results)
