...
__all__ = ["get_file_md5",
           "load_numpy_data", "dump_to_file",
           "read_file",
           "get_loaded_so_path",
...
from .dtypes import unpack_4bits, pack_4bits
...
def load_numpy_data(file_name: str, dtype: str, shape: Union[list, tuple]):
    if not isinstance(dtype, str) and hasattr(dtype, "name"):
        if dtype.name in ("float8_e5m2",) and file_name.endswith(".npy"):
            raise RuntimeError(f".npy file is not supported to be loaded for dtype: {dtype.name}. "
    ...
    if dtype == "uint1":
        ...
        dtype = "float16"
    elif "int4" in str(dtype) or "float4" in str(dtype):
        dtype = "uint8"
    ...
    if "int4" in str(ori_dtype) or "float4" in str(ori_dtype):
        if array.dtype.name in ("int8", "uint8"):
            ...
            if array.size == size:
                ...
                array = array[:size]
        else:  # load from npy file
            array = array.view(ori_dtype)

    special_dtypes = ("bfloat16", "float8_e4m3fn", "float8_e8m0", "hifloat8")
    for sd in special_dtypes:
        if sd in str(ori_dtype) and array.dtype.name != sd:  # load from npy file
            array = array.view(ori_dtype)
    if ori_dtype == "int1":
...
def dump_to_file(data: Union[numpy.ndarray, bytes, "torch.Tensor"],
    ...
    if file_format in ("npy", "print", "pt"):
        ...
        if isinstance(data, numpy.ndarray):
            if "int4" in str(data.dtype) or "float4" in str(data.dtype):
...
def get_loaded_so_path(loaded_cdll: CDLL) -> str:
    ...
    try:
        ...
        results = [o.split(" ")[-1] for o in
                   check_output(["lsof", "-p", str(os.getpid())], encoding="UTF-8").split("\n")
...
def extract_csv_cells(filename, extract_cols,
                      col_name_mapping: dict = None,
                      cmp=None) -> list:
    """
