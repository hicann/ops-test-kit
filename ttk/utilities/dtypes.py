...
def numpy_hifloat4():
    try:
        ...
        raise RuntimeError("en_dtypes is needed to support hifloat4 dtype!!! "
                           "Please install with `pip3 install en-dtypes`")
...
def numpy_hifloat8():
    try:
        ...
        raise RuntimeError("Please upgrade en_dtypes to v0.0.4 at least to support hifloat8 dtype!!! "
...
def pack_4bits(src: numpy.ndarray):
    ...
    if src.size % pack_size != 0:
        array = numpy.pad(src.flatten(),
                          (0, pack_size - src.size % pack_size),
                          mode='constant')
    reshaped = array.reshape([-1, 2])
    # bitwise_and is for arm
    out = numpy.sum(numpy.bitwise_and(reshaped.view(numpy.uint8), 0b00001111) << shift,
                    axis=1, dtype=numpy.uint8)
    return out
...
def encode_float8_e8m0(fp_array: numpy.ndarray):
    ...
    if fp_array.dtype.name not in ("bfloat16", "float16", "float32"):
        raise RuntimeError(f"Dtype of input tensor to be quantized "
                           f"is not supported: {fp_array.dtype.name}")
...
def normalize_to_tf_dtype(np_array: numpy.ndarray):
    ...
    if np_array.dtype.name == "bfloat16" and \
            np_array.dtype.type != tf.bfloat16.as_numpy_dtype.dtype.type:
    ...
    if "bfloat16" in np_dtype:
        ...
        if np_dtype != "float16":
            raise RuntimeError(f"Can only transfer numpy.float16 to torch.complex32 "
                               f"rather than {np_dtype}")
        if not hasattr(torch, "complex32"):
            raise RuntimeError(f"Current pytorch version [{torch.__version__}] is too old. "
                               f"Please update to at least v1.13.1")
...
def torch_to_numpy_tensor(torch_tensor) -> numpy.ndarray:
    ...
    if not isinstance(torch_tensor, torch.Tensor):
        raise RuntimeError(f"Only support torch.Tensor. "
                           f"But got {type(torch_tensor)}")
...
def reshape_scale_array_pad(scale_array: numpy.ndarray, group_index: numpy.ndarray, axis):
    ...
    scale_array = scale_array.reshape(int(scale_array.shape[0]/2), 2, scale_array.shape[1]).transpose(0, 2, 1).reshape(int(scale_array.shape[0]/2), scale_array.shape[1], 2)

    return scale_array


def grouped_mx_quantize(fp_array: numpy.ndarray, group_index: numpy.ndarray, mx_ele_dtype: str = "float8_e5m2",
                        axis: int = -2, block_size: int = 32, round_mode: str = "rint") -> tuple:
    """
