...
| `input_data_ranges` | 否 | ((None,None),) | 每输入随机范围 `[min,max]`（闭区间，min 和 max 为边界值）。None 时 min=-2，max=2。可指定第 3 个及以上的必现值，会混入随机数据中保证出现（小 tensor 约 25% 比例、大 tensor 每值封顶 2048 次）。min==max 时生成固定值。如 `"((-1,1),(0,10))"` 或 `"((-1,1,0),(0,10))"` |
