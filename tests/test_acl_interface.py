...
def test_reset_only_resets_owned_runtime_device(
    ...
    monkeypatch.setattr(device, "_release_acl_memory", lambda: None)
    monkeypatch.setattr(device, "_acl_reset_device", lambda: reset_calls.append(True))
...
@pytest.mark.parametrize(
    ("module_name", "dtype_name", "packed"),
    [
        ("en_dtypes", "hifloat8", False),
        ("en_dtypes", "float8_e8m0", False),
        ("ml_dtypes", "float8_e5m2", False),
        ("ml_dtypes", "float8_e4m3fn", False),
        ("en_dtypes", "float4_e2m1", True),
        ("en_dtypes", "float4_e1m2", True),
        ("ml_dtypes", "int4", True),
    ],
)
def test_create_acl_tensor_from_numpy_preserves_custom_storage_dtype(
        module_name, dtype_name, packed):
    dtype = getattr(pytest.importorskip(module_name), dtype_name)
    storage = numpy.arange(32, dtype=numpy.uint8).view(dtype).reshape(2, 16)
    view = np_as_strided_safe(storage, shape=(2, 2, 4), strides=(16, 4, 1))
    device, captured = make_numpy_tensor_device()

    device.create_acl_tensor(view, "ND", storage.shape)

    copied = captured["copied_array"]
    assert copied.dtype == storage.dtype
    assert copied.nbytes == storage.nbytes
    flattened = captured["flattened_array"]
    expected_bytes = storage.nbytes // 2 if packed else storage.nbytes
    assert flattened.nbytes == expected_bytes
