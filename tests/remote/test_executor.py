...
class TestSpecData:
    ...
    @pytest.mark.parametrize("mode, body, x1, x2, expected", [
        pytest.param(
            "b",
            "class AddImpl:\n"
            ...
            np.array([1.0]), np.array([5.0]), np.array([6.0]),
        ...
        pytest.param(
            ...
            "class AddTestSpec:\n"
            "    third_party = {'numpy': AddImpl}\n",
            np.array([2.0]), np.array([10.0]), np.array([12.0]),
    ...
    def test_spec_input_injection(self, tmp_path, mode, body, x1, x2, expected):
        ...
        env = executor.execute_request(**_kw(
            exec_type="spec", spec_module=f"add{mode}", spec_class="AddTestSpec",
