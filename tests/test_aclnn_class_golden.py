...
class TestAclnnClassGoldenDispatch:
    ...
    def test_partial_split_attr_from_attributes(self, mock_get_plugin, mock_sw, mock_op_info):
        """``scale`` from ``context.attributes`` (not a scalar) also reaches ``__init__``.

        ACLNN plugins may take config via either scalars or attributes; both are merged
        into the pool in ``_named_values``. This guards the attributes-merge half.
        """
        mock_get_plugin.return_value = _AclnnNegScale
        mock_sw.return_value = _mock_switches()
        mock_op_info.return_value.info_of.return_value = _make_op_api_info(tensor_names=["x"])

        inp = np.array([1.0, 2.0, 3.0], dtype="float32")
        case = _make_testcase("aclnnNegScaleAttr",
                              tensors=[inp],
                              attributes={"scale": 5})
    ...
    def test_no_custom_init_hits_cls_guard(self, mock_get_plugin, mock_sw, mock_op_info):
        """``_AclnnDouble`` has no ``__init__`` → ``cls()`` guard path; ``__call__(x)`` → ``x*2``."""
        mock_get_plugin.return_value = _AclnnDouble
        mock_sw.return_value = _mock_switches()
        mock_op_info.return_value.info_of.return_value = _make_op_api_info(tensor_names=["x"])

        inp = np.array([1.0, 2.0, 3.0], dtype="float32")
        case = _make_testcase("aclnnDouble",
                              tensors=[inp],
                              attributes={})
