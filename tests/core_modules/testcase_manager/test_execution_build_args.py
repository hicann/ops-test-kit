...
def _build_positional_args(api_name, nested_tensors, attributes, output_tensor_indexes, tensor_distribution=None):
    ...
    return _real_build_positional_args(
        api_name, nested_tensors, attributes,
        output_tensor_indexes, tensor_distribution, api_info)
...
class TestScoreAttrTypeCompatibility:
    ...
    @pytest.mark.parametrize("api_suffix, attr_name, attr_value, expected_oidx", [
        ("str", "ord", "fro", 1),
        ("int", "ord", 2, 0),
        ("bool", "flag", True, 1),
        ("tuple", "size", (2, 3), 1),
    ], ids=["str-prefers-str-overload", "int-prefers-number-overload",
            "bool-prefers-bool-overload", "tuple-prefers-tuple-overload"])
    def test_attr_type_prefers_matching_overload(self, api_suffix, attr_name,
                                                  attr_value, expected_oidx):
        if api_suffix == "str":
            _register("torch.norm_like_str", [
                [ParamInfo(name="input", type="Tensor"),
            ...
            params, oidx = _match_overload("torch.norm_like_str", 1,
                {attr_name: attr_value, "dim": (-2, -1), "keepdim": False})
        elif api_suffix == "int":
            _register("torch.norm_like_int", [
                [ParamInfo(name="input", type="Tensor"),
                 ParamInfo(name=attr_name, type="Number"),
                 ParamInfo(name="dim", type="tuple of ints", default="(-2,-1)", is_optional=True),
                 ParamInfo(name="keepdim", type="bool", default="False", is_optional=True)],
            ...
            params, oidx = _match_overload("torch.norm_like_int", 1,
                {attr_name: attr_value, "dim": (-2, -1), "keepdim": False})
            ...
            params, oidx = _match_overload("torch.bool_overload_test", 1,
                {attr_name: attr_value, "dim": 0})
        else:
            ...
            params, oidx = _match_overload("torch.tuple_overload_test", 1,
                {attr_name: attr_value, "mode": "bilinear"})
...
class TestCoerceValue:
    ...
    @pytest.mark.parametrize("value, target_type, expected", [
        ...
        "none-passthrough", "string-none",
        "int-string", "float-string", "number-coerces-float",
        "str-passthrough", "int-tuple-fallback", "int-list-fallback",
        "float-tuple-fallback", "float-list-fallback",
        "bool-string-true", "bool-string-false",
        "bool-native-true", "bool-native-false",
        "int-native", "float-native",
    ])
